...
import '../widgets/mind_map_viewer.dart';
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            tooltip: _showOnlyPending ? 'Mostrar Todos' : 'Mostrar Pendentes',
          ),
        ],
      ),
      body: FutureBuilder(
...
class _UploadBottomSheetState extends State<_UploadBottomSheet> {
  ...
  Future<void> _uploadFiles() async {
    ...
    try {
      ...
      await mindMapService.addFiles(widget.subject, widget.topic, files);
      
      if (mounted) {
        Navigator.pop(context);
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('${files.length} mapa(s) adicionado(s) com sucesso!'),
            backgroundColor: AppTheme.successColor,
          ),
        );
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Erro ao enviar arquivos'),
